...
def plot_subimage_rolls2(subimage_roll, subimages, subimage_positions, image_std, image_mean, cmap_name,
    ...
    for s_i, (s_roll, s_image, s_pos) in enumerate(zip(subimage_roll, subimages, subimage_positions)):
        # Unnormalize the image and convert it to an RGB image
        s_image_unznormed = recover_subimage(s_image, s_pos, image_std, image_mean)
        s_image_rgb = Image.fromarray(np.uint8(s_image_unznormed)).convert('RGB')

        # Image 1: Unnormalized image with overlay
        if save_dir is not None:
            combined_image = s_image_rgb.convert('RGBA')
            if np.max(s_roll) > 0:
                overlay = Image.fromarray(np.uint8(255 * (s_roll / np.max(s_roll)))).convert("L")
                overlay_rgba = Image.merge('RGBA', (overlay, overlay, overlay, overlay.point(lambda p: p * overlay_alpha)))
                overlay_rgba = overlay_rgba.resize(combined_image.size)
                combined_image = Image.alpha_composite(combined_image, overlay_rgba)
            combined_image.save(os.path.join(save_dir, f'{notes}_Subimage_{s_i}_with_overlay.png'))

        # Image 2: Unnormalized image without overlay
        if save_dir is not None:
            s_image_rgb.save(os.path.join(save_dir, f'{notes}_Subimage_{s_i}_no_overlay.png'))

        # Image 3: Only the overlay (as grayscale with transparency)
        if save_dir is not None:
            if np.max(s_roll) > 0:
                overlay_rgba = plt.cm.get_cmap(cmap_name)(s_roll / np.max(s_roll))
                overlay_rgba = np.uint8(overlay_rgba * 255)
                overlay_image = Image.fromarray(overlay_rgba)

                # Adjust the alpha channel: 0 where s_roll is 0, otherwise proportional to overlay_alpha
                alpha_channel = np.uint8((s_roll > 0) * (255 * overlay_alpha))
                overlay_image.putalpha(Image.fromarray(alpha_channel))

                overlay_image.save(os.path.join(save_dir, f'{notes}_Subimage_{s_i}_overlay_only.png'))
