...
def get_model(model_name, image_size, depth, device, *args, **kwargs):
    ...
    elif model_name == 'vit_small_patch16_224_in21k_subimage':
        model = timm.create_model(model_name.replace('_subimage', ''),  pretrained=True, num_classes=2, dynamic_img_size=True)  # weights from 'https://storage.googleapis.com/vit_models/augreg/L_16-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.1-sd_0.1.npz', official Google JAX implementation
        model = ExtensionTimmViTSubimage(model).to(device)
...
def get_subimage_model(precompute: Union[List[str], str]=None, *args, **kwargs):
    ...
    return subimage_handler
#TODO write this in 10 mins

def get_subimage_model2(precompute: Union[List[str], str]=None, *args, **kwargs):
    """
    Get the subimage handler with the precomputed results
    ...
    results_dir = '/data/kuang/David/ExpertInformedDL_v3/temp/results-bscan-07_17_2024_00_50_17'
    data_path = '/data/kuang/David/ExpertInformedDL_v3/bscan_v2.p'
    ...
    models = list(reversed(list(models)))
    model, model_results, model_config_string = get_best_model(models, results_dict)
    model.to(device)
    model.eval()

    subimage_handler.models['vit'] = model
    subimage_handler.compound_label_encoder = pickle.load(open('/data/kuang/David/ExpertInformedDL_v3/temp/results-bscan-07_17_2024_00_50_17/compound_label_encoder.p','rb'))
    ...
    for j, image_name in enumerate(subimage_handler.image_data_dict.keys()):
        #print(f"Precomputing {model_name} ({i}/{len(precompute)}) for {image_name}, {j}/{len(subimage_handler.image_data_dict.keys())}", end='\r', flush=True)
        subimage_handler.compute_perceptual_attention(image_name, model= model, model_name='vit', is_plot_results=False, *args, **kwargs)
    with open('subimage_model2_draft.pkl', 'wb') as f:
